...
"""
Build paper-ready post-drift analysis figures and CSV outputs.

Key defaults for paper figures
------------------------------
1. Only the 5 main methods are kept:
   - ADAWU-IDS
   - DWM
   - Online Bagging
   - Leveraging Bagging
...
2. All Static LSTM seed runs are automatically aggregated into one method.

3. Three clean figure outputs are generated by default:
...
def normalize_method_name(raw: str) -> str:
    ...
    mapping = [
        ("ADAWU-IDS", [
            " adawu_ids ", " adawu ", " paper_trace ", " adawuids ",
            " adawu_trace ", " adawu_id "
        ]),
        ("DWM", [" dwm "]),
        ...
        ("Static LSTM", [
            " static_lstm ", " static ", " static_predictions ",
            " static_prediction ", " lstm_static ", " static_baseline "
        ]),
    ]
...
def parse_seed_from_name(path: Path) -> int:
    ...
    patterns = [r"seed[_\-]?(\d+)", r"run[_\-]?(\d+)", r"s[_\-]?(\d+)"]
    for pat in patterns:
...
def plot_aligned_recovery(aligned_agg: pd.DataFrame, out_path: Path, title: str, smooth_window: int, show_ci: bool) -> None:
    ...
    if aligned_agg.empty:
        ...
        plt.xlabel("Relative Step After Drift")
        plt.ylabel("Weighted F1")
        plt.tight_layout()
        plt.savefig(out_path, dpi=220, bbox_inches="tight")
        plt.close()
...
def plot_summary_bars(stats: pd.DataFrame, out_path: Path) -> None:
    ...
    _bar_panel(axes[1, 1], ordered, "recovery_time", "Recovery Time (chunks)")
    plt.tight_layout()
    plt.savefig(out_path, dpi=220, bbox_inches="tight")
    plt.close()
