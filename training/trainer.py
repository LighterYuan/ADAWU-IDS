...
from configs.data.data_processor import CICIDSDataProcessor
...
class ModelTrainer:
    ...
    def prepare_data(self, data_dir=None, use_processed=True, sample_fraction=None):
        """
        Prepare training datasets with consistent dimension handling
        """
        processed_dir = Config.PROCESSED_DATA_DIR

        # Check if processed datasets exists
        ...
        if processed_data_exists and sample_fraction is None:
            ...
            try:
                ...
                print("Falling back to raw datasets processing...")

        # If we get here, either use_processed is False, processed datasets doesn't exist, or sampling is requested
        if data_dir and os.path.exists(data_dir):
            # Preprocess from raw datasets
            print("Preprocessing from raw datasets...")
            ...
            if raw_data is not None:
                ...
                # Save preprocessed datasets (only if no sampling)
                ...
                raise ValueError("Unable to load raw datasets")
        else:
    ...
    def train_basic_model(self, train_data, val_data, class_weight=None, epochs=None, batch_size=None):
        ...
        # Calculate class weights (handle imbalanced datasets)
